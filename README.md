...
**Start here:** [`docs/TECHNICAL_SUMMARY.md`](docs/TECHNICAL_SUMMARY.md), the concise technical document in the brief's order (one short section per required item, each linking to its depth in `docs/TECHNICAL.md`).
...
**v1.1 (experiment L, `docs/experiments/L-final.md`).** Three decisions were made in the scoring and retrieval layers, each by a rule written before it ran and on the train-carved validation split: the stated touristiness preference now steers the served ranking (`pref_align`, `docs/TECHNICAL.md` section 3.1), the MMR lambda was selected instead of defaulted, and the retrieval design was re-decided (the learned retriever stays). Gate-B's blocking rows were amended before the retrieval comparison ran. Tags: `v1.0-konnect-submission` is the first submission, `v1.1-konnect-submission` adds experiment L, and `v1.2-konnect-submission` is v1.1 plus the documentation-only technical summary (`results/metrics.json` byte-identical); none was moved.
...
`v1.2-konnect-submission` was cloned into a clean directory, dependencies installed with
...
of that final run are recorded in the annotated tag (`git show v1.2-konnect-submission`), because a commit
...
docs/            TECHNICAL_SUMMARY.md, TECHNICAL.md, RESULTS.md, REQUIREMENTS.md, DATA_CARD.md (+ the .md.tmpl sources)
